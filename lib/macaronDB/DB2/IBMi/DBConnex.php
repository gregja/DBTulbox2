 ...
 * @category   MacaronDB
 * @package    DB
 * @copyright  Copyright (c) 2012 Gregory Jarrige
 * @author     Gregory Jarrige <gregory_jarrige@yahoo.fr>
 * @license    New BSD License
 * @version    DB/DB2/IBMi/DBConnex.php
 */
...
class DB2_IBMi_DBConnex {
	...
	public static function generate_config ($options=array()) {
		...
		if (isset($options ['i5_libl']) ) {
			// tableau à transformer en chaîne de caractères si nécessaire + forcage de la syntaxe IBM
			$options ['i5_naming'] = DB2_I5_NAMING_ON;
		...
		if (! isset ( $options ['DB2_ATTR_CASE'] ) && ! isset ( $options ['db2_attr_case'] )) {
			...
			// (exemples : DB2_CASE_UPPER, CASE_UPPER, *UPPER, ou UPPER, en majuscules ou minuscules), on normalise les valeurs é
