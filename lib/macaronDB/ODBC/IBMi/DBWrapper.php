 ...
 *
 * Wrapper Base de Données dédié au connecteur odbc_connect
 */
...
abstract class ODBC_IBMi_DBWrapper implements DBWrapperInterface {
     ...
     * le "/" au lieu du "."
     * comme séparateur entre nom de bibliothéque et nom de table. Donc pour
     * savoir si le "/" ou le "."
     * doit être utilisé, on se base sur le contenu du paramètre "i5_naming"
     * défini dans les options de odbc_connect.
     */
    ...
    public static function getPagination($db, $sql, $args, $offset,
        ...
        // TODO : à simplifier au profit de la clause LIMIT apparue en V7
        $sql = <<<BLOC_SQL
select foo.* from (
{$sql}
) as foo
...
BLOC_SQL;
     ...
     * contréle sur la présentation des erreurs renvoyées par cette classe
     * @param type $db
    ...
    private static function MyDBError($db, $fonction_db2, $reqsql, $args = array()) {
        ...
        if (isset($GLOBALS['flag_sql_debug']) && $GLOBALS['flag_sql_debug'] === true) {
            echo "<table border=\"1\">
       <tr>
        <td> Code SQL </td>
        <td> {$reqsql} </td>
